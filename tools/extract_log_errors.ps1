<#
.SYNOPSIS
    Extracts WorkplaceTriggers-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the WorkplaceTriggers mod, categorizing them by severity and type.
...
Write-Header "WorkplaceTriggers Log Analyzer"
...
# Patterns to match
$patterns = @{
    # WorkplaceTriggers specific patterns
    WorkplaceTriggersLine = 'WorkplaceTriggers|FS25_WorkplaceTriggers'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
    ...
    # Specific WorkplaceTriggers patterns
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is WorkplaceTriggers related
    if ($line -match $patterns.WorkplaceTriggersLine) {
        $stats.WorkplaceTriggersLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'WorkplaceTriggers') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report = @()
$report += "=" * 60
$report += "WorkplaceTriggers Error Report"
...
$report += "WorkplaceTriggers-Related Lines: $($stats.WorkplaceTriggersLines)"
